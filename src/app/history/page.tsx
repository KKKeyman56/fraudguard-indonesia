            ...
            {history.items.map((item) => {
              ...
              return (
                <article className="neon-card history-card" key={item.id}>
                  <div className="history-score" data-risk={riskLabelClass(label)}><strong>{item.overallRisk}</strong><span>RISIKO / 100</span></div>
                  <div className="history-detail">
                    ...
                    <div className={`status ${riskLabelClass(label)}`}><ShieldCheck size={13} /> {label}</div>
                    <p>{item.aiSummary}</p>
                    <div className="history-counts"><span className="safe">{item.aman} aman</span><span className="warning">{item.waspada} waspada</span><span className="danger">{item.risikoTinggi} risiko tinggi</span></div>
                  </div>
