...
import { Activity, ArrowRight, Bot, Database, GitCompareArrows, ReceiptText, ShieldCheck, UserCog, Users } from "lucide-react";
...
export default async function AdminPage() {
  ...
  const stats = [
    ...
    { label: "Risiko tinggi", value: data.risk.detected, note: `${percent(data.risk.detected)}% dari transaksi`, icon: ShieldCheck },
  ];

  return <main className="app-page grid-bg admin-page">
    <div className="page-intro admin-intro"><div><span className="eyebrow">ADMIN COMMAND CENTER // RESTRICTED</span><h1>Dashboard admin</h1><p>Pantau pertumbuhan pengguna, aktivitas analisis, dan distribusi risiko FraudGuard.</p></div><div className="admin-intro-actions"><span className="admin-verified"><ShieldCheck size={16} /> ADMIN TERVERIFIKASI</span><Link className="button button-secondary button-small" href="/admin/payments"><ReceiptText size={16} /> Pembayaran</Link><Link className="button button-secondary button-small" href="/admin/model"><GitCompareArrows size={16} /> Risk Engine</Link><Link className="button button-secondary button-small" href="/admin/users"><UserCog size={16} /> Kelola pengguna</Link></div></div>
    ...
    <section className="admin-grid">
      <article className="neon-card admin-risk-card"><div className="admin-section-title"><div><span className="eyebrow">RISK DISTRIBUTION</span><h2>Status seluruh transaksi</h2></div><Activity size={27} /></div><div className="risk-bars"><div><p><span>AMAN</span><strong>{data.risk.safe} · {percent(data.risk.safe)}%</strong></p><i><b className="safe" style={{ width: `${percent(data.risk.safe)}%` }} /></i></div><div><p><span>WASPADA</span><strong>{data.risk.warning} · {percent(data.risk.warning)}%</strong></p><i><b className="warning" style={{ width: `${percent(data.risk.warning)}%` }} /></i></div><div><p><span>RISIKO TINGGI</span><strong>{data.risk.detected} · {percent(data.risk.detected)}%</strong></p><i><b className="danger" style={{ width: `${percent(data.risk.detected)}%` }} /></i></div></div><p className="admin-note">Statistik adalah alat pemantauan. Keputusan final terhadap transaksi tetap memerlukan verifikasi manusia.</p></article>
    ...
    <section className="neon-card admin-activity-card"><div className="admin-section-title"><div><span className="eyebrow">LATEST ACTIVITY</span><h2>Analisis terbaru</h2></div><span className="admin-table-note">8 aktivitas terbaru</span></div>{data.recentRuns.length === 0 ? <div className="empty-state">Belum ada aktivitas analisis.</div> : <div className="table-wrap"><table className="admin-table"><thead><tr><th>Waktu</th><th>Pengguna</th><th>Sumber</th><th>Model AI</th><th>Risiko</th><th>Laporan</th></tr></thead><tbody>{data.recentRuns.map((run) => { const label = riskLabel(run.overallRisk); return <tr key={run.id}><td>{formatDate(run.createdAt)} WIB</td><td>{run.email}</td><td>{run.source === "manual" ? "Input manual" : "Upload file"}</td><td>{run.model}</td><td><span className={`status ${riskLabelClass(label)}`}>{run.overallRisk}/100 · {label}</span></td><td><Link className="text-link" href={`/report?id=${run.id}`}>Lihat <ArrowRight size={14} /></Link></td></tr>; })}</tbody></table></div>}</section>
  </main>;
}
